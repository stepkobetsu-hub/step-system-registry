...
- 現行デプロイ: 既存デプロイIDを維持し、2026-08-19にApps Scriptバージョン56「全県模試受験票・既存生の中学校コード自動照合」へ更新。
...
- 印刷内容: 第1回～第6回のタイトル、塾名、塾コード4258、受験番号、3桁の中学校コード＋学校名（例：353（南城中））、氏名＋小さめの学年表記（例：山田太郎（中１））、学区「尾張」、年度、校舎名、学年別時間割、イラスト。選択した全生徒を1人1枚のA4で一括印刷する。小学生の中学校コード欄は対象外として「—」を表示する。
...
- 訂正導線: 画面右上の「受験番号を訂正する」から `全県模試受験番号`、「中学校コードを訂正する」から `中学校コード` シートを開く。
