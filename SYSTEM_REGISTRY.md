...
| STEP配信システム | 本番使用中（履歴は直近10件版） | https://stepkobetsu-hub.github.io/step-message-center/ | [step-message-center](https://github.com/stepkobetsu-hub/step-message-center) | `main`（開封確認撤去 `159cecd`） | `index.html`、`app.js`、`api.js`、`Code.gs`、Apps Script Webアプリ v53 | GitHub＋Apps Script＋Brevo | Pagesを更新し、`Code.gs`変更時は既存デプロイIDを維持して新バージョンへ更新 | 2026-08-13 | 履歴タブを開くと直近10件を新しい順に自動取得。検索／クリア横に更新ボタン。開封確認機能は利用者判断で撤去し、Brevo Webhook・専用Script Properties・「開封キャッシュ」シートも削除済み |
