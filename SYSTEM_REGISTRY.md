...
| STEP配信システム | 本番使用中（履歴画面は直近10件版） | https://stepkobetsu-hub.github.io/step-message-center/ | [step-message-center](https://github.com/stepkobetsu-hub/step-message-center) | `main`（画面 `2a9a183`／GAS高速化 `1b2a595`） | `index.html`、`app.js`、`api.js`、`Code.gs`、Apps Script Webアプリ | GitHub＋Apps Script＋Brevo | Pages更新済み。`Code.gs`変更時は既存Webアプリを新バージョンへ更新し、デプロイIDを維持 | 2026-08-13 | 送信履歴タブで条件解除・自動取得。検索／クリア横に更新ボタン。直近10件のみ表示。GAS高速化の本番反映は既存デプロイ更新待ち |
