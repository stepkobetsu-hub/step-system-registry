...
- **予定表示:** 時計下部には枠や「予定」の見出しを置かず、今日・明日の2日分のGoogleカレンダー予定を大きな文字でゆっくり上下に流して表示する。表示部分を押すと、日付を押した場合と同じ大画面で今日・明日の予定一覧を開く。予定がなければ「-」。日付のみ表示では、予定がある日だけ日付の下に点を表示する。予定日一覧は閲覧専用Apps Scriptから取得し、端末へ短時間キャッシュする。
...
- **確認日:** 2026年9月16日。Sites版17を本番公開済み。
