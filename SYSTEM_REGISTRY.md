...
- Apps Script: 非公開の本番プロジェクト、バージョン43。既存の本番デプロイIDを維持
...
- メール処理: 入退室保存と通知メールを分離。送信待ち・送信済み・送信失敗をキューへ保存し、受付IDを冪等性キーとして二重メールを防止。メール失敗でも保存済みの入退室記録は受付成功として保持。Brevo APIキー設定時はBrevo、未設定時は緊急用MailAppフォールバックを使用し、送信方式を画面・匿名化ログ・キューへ記録
- 講師通知: 講師マスタP列（列16、行配列index 15）を取得。見出し検出を優先し、見出し不一致時はP列を検証して使用。講師索引キャッシュは `v43-email-p15` へ更新し、旧キーを削除
- メール復旧検証: 外部送信を抑止したダミーキュー4件（生徒／講師×写真なし／あり）はPENDING残存0、二重キュー0。承認済み内部テスト宛先へMailAppフォールバックで写真付き1通の実送信に成功。同一テストの再実行をプロパティで禁止
...
- GitHub: メール復旧PR #5、mainマージSHA `e42952108d23b9c79e44d94e0fd8d6f581935597`
- 確認日: 2026-08-04
