...
|---|---|---|---|---|---|---|---|---|---|
| デジタルカレンダー | 本番公開中（Fire 7／Windows 10・11） | https://fire-digital-calendar.mintcocoajasmine.chatgpt.site | ChatGPT Sites管理ソース＋[step-system-registry/windows-calendar](https://github.com/stepkobetsu-hub/step-system-registry/tree/main/windows-calendar) | `main` | ChatGPT Sites管理ソース、`dist/`一式、Windows版の検証用ソース | ChatGPT Sites＋Googleカレンダー閲覧専用Apps Script連携 | Web版はChatGPT Sitesで新版を公開。WindowsではMicrosoft Edgeで本番URLを開き、「アプリ」→「このサイトをアプリとしてインストール」を使用 | 2026-09-16 | Windows用の署名なしEXEはSmart App Controlにブロックされたため正式配布に使用しない。Smart App Controlは無効化せず、Edgeのサイトアプリを正式方式とする。専用ウィンドウ、デスクトップ／スタートメニュー、自動起動の選択が可能で、Web更新時の再インストールは不要 |
