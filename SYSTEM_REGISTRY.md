...
| 請求システム | **Cloudflare完全統合PR確認待ち（本番送信は無効）** | **請求管理:** https://stepkobetsu-hub.github.io/invoice-pdf/#adjustments<br>**請求書配信・PDF作成:** https://stepkobetsu-hub.github.io/invoice-pdf/<br>**旧料金調整互換URL:** https://stepkobetsu-hub.github.io/seiseki-kanri/billing_adjustment.html | [invoice-pdf](https://github.com/stepkobetsu-hub/invoice-pdf) | `main`（統合PR [#28](https://github.com/stepkobetsu-hub/invoice-pdf/pull/28) 未マージ） | `index.html`、`assets/app.js`、`cloudflare/src/index.js`、`cloudflare/migrations/0008_complete_invoice_integration.sql`、`docs/ISSUE_27_CLOUDFLARE_INTEGRATION_REPORT_2026-09-02.md` | GitHub Pages＋Worker `step-invoice-api`＋D1 `step-invoice-db`＋非公開R2 `step-invoice-pdfs`。Google Sheetは移行確認期間のバックアップ／互換mirror | PRでは自動テストのみ。main反映時に非公開D1 backup→migration→Worker deploy。取引先・設定・料金調整・請求書・領収書・入金・配信状態はD1、PDFはR2を正本とする。本番送信は管理者の別承認まで拒否 | 2026-09-02 | 既存URLを維持し、旧料金調整ページは新画面へ互換転送するPR [seiseki-kanri #17](https://github.com/stepkobetsu-hub/seiseki-kanri/pull/17) を作成。`PRODUCTION_SEND_APPROVED=false` と `TEST_SEND_APPROVED=false` を維持。ローカルmigration 0001〜0008・全自動テスト・本番health成功。リモート件数・金額照合はPRマージ前の保護工程で実施 |
