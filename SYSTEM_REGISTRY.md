...
- 印刷内容: 第1回～第6回のタイトル、塾名、塾コード4258、受験番号、中学校名と3桁の中学校コード、氏名＋学年（例：山田太郎（中１））、学区「尾張」、年度、校舎名、学年別時間割、イラスト。選択した全生徒を1人1枚のA4で一括印刷する。小学生の中学校コード欄は対象外として「—」を表示する。
...
- 本番確認: 2026年度APIで在籍生90名・中学生65名を取得し、登録済み学校コードを既存中学生64名へ反映、未登録校1校を登録画面に表示、API警告0件・受験番号重複0件であることを2026-08-19に確認。フロントエンドは `exam_ticket.js?v=20260819-10`／`exam_ticket.css?v=20260819-6`。中学校コード見出しの1行固定と、氏名欄の大型化・学年併記を確認。Apps Scriptはv56。
