...
| 出退くんQR作成・読取 | 本番使用中（講師メールP列対応 Apps Script v21／QR機能維持） | **管理者QR登録:** https://stepkobetsu-hub.github.io/student-QR/student_qr_register.html<br>**講師QR作成:** https://stepkobetsu-hub.github.io/student-QR/teacher_qr_create.html<br>**塾生用QR:** https://stepkobetsu-hub.github.io/student-QR/my_qr.html<br>**タブレット読取:** https://step-checkin-edge-staging.stepkobetsu.workers.dev/legacy-tablet<br>**Apps Script本番:** https://script.google.com/macros/s/AKfycbzYpm-16ahuZ3BRFKRT-iSvR9nThsYcTOhxplyBp4bZmVmehfTYZEEl18THzJasypOsTQ/exec | [student-QR](https://github.com/stepkobetsu-hub/student-QR) | `main`（講師メール対応 `ea46ba9d22148506a371801b21c5240f33dd9278`） | `teacher_qr_create.html`、`student_qr_register.html`、`my_qr.html`、`tablet_checkin.html`、`cloudflare/checkin-edge/`、Apps Script project `1jZRwuaEqbhgg6xRQq63ke5QO9Wc2ulsGOA_gbmHfiehQIsr9NQLLqSZR` の `コード.js`、Google Sheet `給与明細2026-6-` の `講師マスター`（A列=講師コード、B列=氏名、P列=メール、Q列=QR） | GitHub Pages＋Cloudflare Workers/Durable Objects＋Apps Script v21＋Google Sheet | 画面は `student-QR/main` を更新。GASは[正本編集画面](https://script.google.com/home/projects/1jZRwuaEqbhgg6xRQq63ke5QO9Wc2ulsGOA_gbmHfiehQIsr9NQLLqSZR/edit)を `clasp pull` で取得し、既存デプロイID `AKfycbzYpm-16ahuZ3BRFKRT-iSvR9nThsYcTOhxplyBp4bZmVmehfTYZEEl18THzJasypOsTQ` を指定して新バージョンへ更新する。新しいデプロイは作らず `/exec` URLを維持 | 2026-08-31 | Issue [#43](https://github.com/stepkobetsu-hub/student-QR/issues/43)で講師メールをP列正本へ統一。未登録は赤字「メールアドレス未登録」。講師ポータルへのフォールバックとGitHubへのメール実値保存は禁止。登録済み／未登録、QR表示・発行・印刷、正本・デプロイ手順は `docs/student-qr-teacher-email-20260831.md` を参照 |
...
| 請求システム | **Cloudflare完全統合・本番稼働中（本番送信は無効）** | **請求管理:** https://stepkobetsu-hub.github.io/invoice-pdf/#adjustments<br>**請求書配信・PDF作成:** https://stepkobetsu-hub.github.io/invoice-pdf/<br>**旧料金調整互換URL:** https://stepkobetsu-hub.github.io/seiseki-kanri/billing_adjustment.html | [invoice-pdf](https://github.com/stepkobetsu-hub/invoice-pdf) | `main`（統合PR [#28](https://github.com/stepkobetsu-hub/invoice-pdf/pull/28)、配備修正PR [#29](https://github.com/stepkobetsu-hub/invoice-pdf/pull/29) 反映済み） | `index.html`、`assets/app.js`、`cloudflare/src/index.js`、`cloudflare/migrations/0008_complete_invoice_integration.sql`、`docs/ISSUE_27_CLOUDFLARE_INTEGRATION_REPORT_2026-09-02.md` | GitHub Pages＋Worker `step-invoice-api`＋D1 `step-invoice-db`＋非公開R2 `step-invoice-pdfs`。Google Sheetは移行確認期間のバックアップ／互換mirror | 取引先・設定・料金調整・請求書・領収書・入金・配信状態はD1、PDFはR2を正本とする。main反映時に自動テストとWorker deployを実行し、healthでD1 schemaを安全に初期化・確認。本番送信は管理者の別承認まで拒否 | 2026-09-02 | Worker Version `ac8bdf45-5960-43e6-9a3e-70bf140ef94c`。Actions run 33620220615成功、公開health・請求画面・旧URL互換転送を確認。`PRODUCTION_SEND_APPROVED=false` と `TEST_SEND_APPROVED=false` を維持。D1 REST backup/migrationは現行API tokenの権限不足のため警告継続だが、Worker binding経由の冪等schema初期化で本番稼働済み |
