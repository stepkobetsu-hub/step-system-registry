...
| STEP配信システム | 本番使用中（直近10件・開封確認高速版） | https://stepkobetsu-hub.github.io/step-message-center/ | [step-message-center](https://github.com/stepkobetsu-hub/step-message-center) | `main`（開封確認高速化 `860553d`） | `index.html`、`app.js`、`api.js`、`Code.gs`、Apps Script Webアプリ v52 | GitHub＋Apps Script＋Brevo | Pagesを更新し、`Code.gs`変更時は既存デプロイIDを維持して新バージョンへ更新。開封Webhookは `setupStepBrevoOpenWebhook`、既存開封データ移行は `syncStepOpenCacheFromSharedLog` を実行 | 2026-08-13 | 履歴タブを開くと直近10件を新しい順に自動取得。開封確認あり／なし／データなしを表示。外部共通ログ2,000行を毎回読む方式を廃止し、同一スプレッドシートの小型「開封キャッシュ」を参照。Apps Script v52・既存データ同期済み。画像遮断等により未開封を断定しない |
