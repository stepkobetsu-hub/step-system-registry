...
- 共有設定内の追加カードはカードIDで識別する。同じURLや同じページ内のハッシュを使うカードでも、IDが異なれば重複排除せず表示する。これにより7項目・39カードを全件表示する。
