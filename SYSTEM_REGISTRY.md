...
| 請求システム | 本番使用中 | https://script.google.com/macros/s/AKfycbxzkE1tQRyB_Ca4bfPKYWIkpTukIVPMWKf2ETE7yN7qROJk0VyOlvxaJ9GGI5p-6pGb/exec | 請求管理：GitHub正本なし（Apps Script管理）<br>PDF作成・配信：[invoice-pdf](https://github.com/stepkobetsu-hub/invoice-pdf) | 請求管理：該当なし<br>PDF作成・配信：`main`（最新確認 `c9f00a58`） | **請求管理システム:** Apps Script `請求システム2026NEW`、project `1FQElz87j5yB-FNwuDE9LJ3_nD8rzF_vIGTTWKDr15KDygGxXnZLlXhIp`、`コード.gs`、`BillingV31_Index.html`、`BillingV31_Auth.gs`、`BillingV31_InvoiceMail.gs`<br>**STEP請求書PDF作成・配信システム:** `index.html`、`assets/`、`apps-script/Code.gs`、`cloudflare/`、Apps Script `コード_v023.gs`、`Download.html`、`appsscript.json` | 請求管理：Google SheetバインドApps Script<br>PDF作成・配信：GitHub Pages＋Apps Script＋Google Sheet＋Cloudflare Worker/D1/非公開R2 | 請求管理は[Apps Script編集画面](https://script.google.com/home/projects/1FQElz87j5yB-FNwuDE9LJ3_nD8rzF_vIGTTWKDr15KDygGxXnZLlXhIp/edit)、PDF作成・配信は[専用Apps Script正本](https://script.google.com/home/projects/1SnTqPE8bSQKLkiJI6rPo-7WGQDZoqGpwY7LAAox3FFsj3sGstnHf41X1/edit)とinvoice-pdf mainを更新。両システムのコード・設定を混在させない | 2026-09-02 | 公開台帳では1枚のカードに統合。カード表面には「請求管理システムを開く」「請求書：作成・配信システムを開く」（https://stepkobetsu-hub.github.io/invoice-pdf/）の2入口だけを明確に表示する。請求書画面では左メニューと請求書一覧の境界をドラッグして幅を調整・保存できる。内部システムは別基盤のまま維持し、Cloudflare版コードやPDF配信用Script Propertiesを請求管理側へ設定しない |
