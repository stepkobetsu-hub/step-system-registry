...
| STEP配信システム | 本番使用中（欠席一覧の低負荷自動更新・当日分はA列タイムスタンプ新着順） | https://stepkobetsu-hub.github.io/step-message-center/ | [step-message-center](https://github.com/stepkobetsu-hub/step-message-center) | `main`（現行 `c74ae4d`／送信ログ操作不能修正 `c74ae4d`／当日新着順 `81905b5`／キャッシュ更新 `39882d1`） | `index.html`、`app.js`、`api.js`、`style.css`、`Code.gs`、Apps Script Webアプリ v53（今回変更なし）。欠席・遅刻・早退の回答正本：[遅刻・欠席・早退連絡（回答）](https://docs.google.com/spreadsheets/d/1c2He5p_FMXGq0Gor74wIrJKtdBvTdjmO992ZkNSVuLQ/edit)、Spreadsheet ID `1c2He5p_FMXGq0Gor74wIrJKtdBvTdjmO992ZkNSVuLQ`、参照シート `★欠席遅刻` | GitHub Pages＋既存Apps Script v53＋Brevo＋Google Sheet | 画面変更はGitHub `main`へ反映してPages成功を確認。API変更時だけApps Script既存デプロイIDを維持して新版へ更新 | 2026-09-08 | 送信ログが多い端末でも一覧を画面内スクロールでき、右上×・下部ボタン・背景クリック・Escキーで閉じられるよう修正。送信処理、保存データ、Google Sheet、Apps Script v53は変更なし。 |
...
- **予定表示:** 時計下部には枠や「予定」の見出しを置かず、本日を含む3日分（設定で1～7日）のGoogleカレンダー予定を「月日（曜日）開始時刻-　タイトル」の順に1行の大きな文字で、ヘッドラインニュースのように右から左へゆっくり流して表示する。表示部分を押すと、選択した日数分の予定一覧を大画面で開く。予定がなければ「-」。日付のみ表示では、予定がある日だけ日付の下に点を表示し、日本の祝日は日付の数字を赤色にする。予定日一覧・予定タイトル・祝日は閲覧専用Apps Scriptから取得し、端末へ短時間キャッシュする。日付タップ、流れる予定のタップ、「スケジュール」表示はいずれもGoogleログイン不要のアプリ内一覧を使用し、Android 7など古い端末のGoogleログインループを回避する。「スケジュール」表示は上下スライドで月末まで閲覧できる。ヘッドライン取得に一時失敗した場合は直前の予定を残して自動再取得する。
- **カレンダー操作:** 日付カレンダーは上下・左右スワイプで前月・次月へ移動する（上・左で次月、下・右で前月）。令和年の右にある「今日」ボタンで当月へ戻り、5分間操作がなければ自動でも当月へ復帰する。年月、令和年（年月の右側）、曜日、日付、時計、予定はFire 7で読める大きさを優先する。
...
- **確認日:** 2026年9月16日。Sites版26を本番公開済み。
