...
### 講師初期登録・登録情報変更（2026-09-12）

- [公開アプリ](https://stepkobetsu-hub.github.io/teacher-portal/registration.html)を講師ポータルへ追加。講師番号＋パスワードで本人の行を後日追加入力・修正できる。
- 初回は姓・名、フリガナ、メールとパスワードを登録。銀行口座・住所・生年月日・マイナンバーは任意で後日追加可。初回・パスワード設定／再設定のコードは教室スタッフが発行（7日・1回限り）。
- A自動採番（現状7092、次回7093）／D=1／G=登録日時／MはL生年月日から年齢算出。B・Cは姓と名を空白区切りで保存。銀行口座の先頭0を保持し、ゆうちょは9900・記号・種別空欄・番号。
- 既存講師の同じ行だけ更新。未変更項目、A・D・G・N・Q以降は維持。マイナンバーは再表示しない。パスワードはシートに保存しない。
- 正本：teacher-portal main `60b50a4e8b0e4174d0e3e14500438aea55cafaac`。登録用Apps Script「BREVO_API_KEY」v27（20:33 JST）、既存デプロイURLを維持。student-QR `16ea6b6f497077bee3fb31654cfe745f7412714a`へAPI正本も同期。通知側「生徒QR」v87は未変更。
- 自動テスト9群（採番・本人認証・後日更新・二重送信・競合・パスワード再設定・中断復旧など）と公開画面を確認。本番へ架空講師行を追加せず、実講師の初期登録・追加入力は本人による確認待ち。
- [操作・管理手順](https://github.com/stepkobetsu-hub/teacher-portal/blob/main/docs/teacher-registration.md)。
