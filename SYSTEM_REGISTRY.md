...
| 遅刻・欠席・早退連絡 | 本番使用中（管理者メール一覧の2段表示対応） | https://stepkobetsu-hub.github.io/step-form/ | [step-form](https://github.com/stepkobetsu-hub/step-form) | `main`（件名対応 `632b5de`） | `index.html`、Apps Script project `1d5p77ukBLgeWBJ0cKwCB0ZUui4iVkI-IMIeJqp_15GH3QtzByv3O6xGC`、[Apps Script編集画面](https://script.google.com/u/0/home/projects/1d5p77ukBLgeWBJ0cKwCB0ZUui4iVkI-IMIeJqp_15GH3QtzByv3O6xGC/edit)、[回答Google Sheet](https://docs.google.com/spreadsheets/d/1c2He5p_FMXGq0Gor74wIrJKtdBvTdjmO992ZkNSVuLQ/edit)、`★欠席遅刻` | GitHub Pages＋Google Apps Script Webアプリ＋Google Sheet | 画面変更はGitHub `main`、メール・保存処理はApps Script編集画面から既存デプロイIDを維持して更新 | 2026-08-25 | 管理者メール一覧：件名は`氏名【神領／大手】連絡内容`、本文1行目（一覧プレビュー）にも同じ連絡内容を表示する。連絡内容は`欠席`、`遅刻:5～15分`、`遅刻:15～30分`、`遅刻:30分以上`、`早退希望`。`新しい連絡が届きました`は表示しない。実行URLは既存Webアプリを維持 |
...
- 回答正本シート: `★欠席遅刻`
- 2026-08-25管理者メール一覧仕様: 件名は`氏名【神領／大手】連絡内容`、本文1行目にも同じ連絡内容を表示する。連絡内容は`欠席`、`遅刻:5～15分`、`遅刻:15～30分`、`遅刻:30分以上`、`早退希望`とし、`新しい連絡が届きました`は削除する
- GitHub件名対応コミット: `632b5de74e1108b41555b68932d164e5e9442bd5`
