...
| フォレスタ進捗管理 | **V3本番稼働中（管理者一覧もSupabase高速経路・旧GASロールバック可）** | https://stepkobetsu-hub.github.io/foresta-progress-v2/<br>緊急時: https://stepkobetsu-hub.github.io/foresta-progress-v2/?legacy=1 | [foresta-progress-v2](https://github.com/stepkobetsu-hub/foresta-progress-v2) | `main`（本番 `cf3fc3b0f26a4dae985fb40225b6d16ddfe0510a`） | `app.js`、`config.js`、`apps-script/Code.gs`、Supabase Edge Functions `foresta-runtime-v3` / `foresta-timetable-sync`、`foresta_v3_*` テーブル | GitHub Pages＋Supabase V3＋Apps Script v19＋旧Google Sheet（ロールバック・非同期ミラー用） | 通常の生徒・講師・管理者読込はSupabaseスナップショット。管理者 `getAdminDashboard` / `getAdminStudents` は `__global__` スナップショットを使用し、GASを画面表示のホットパスへ戻さない。通常保存はmutation queueへ即時受付後に非同期GASミラー。`?legacy=1` のみ旧GAS経路 | 2026-09-01 | PR #11をmainへ反映しPages run 33404387829成功。1001 加瀬壮真（小5）・1320 加瀬智子（中2）で本番確認。管理者速報は約14.5秒→約6.9秒、全生徒は約17.3秒→約2.9秒。小学生専用宿題ページ、学校単元テストUI、役割色、生徒/講師既存高速経路を維持。`npm test` 全件成功。詳細 `docs/foresta-progress-v3-production-20260831.md` |
