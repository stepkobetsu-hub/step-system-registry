...
| STEP配信システム | 本番使用中（直近10件・開封確認版） | https://stepkobetsu-hub.github.io/step-message-center/ | [step-message-center](https://github.com/stepkobetsu-hub/step-message-center) | `main`（開封確認 `d41988b`／Webhook補正 `89061a1`） | `index.html`、`app.js`、`api.js`、`Code.gs`、Apps Script Webアプリ v51 | GitHub＋Apps Script＋Brevo | Pagesを更新し、`Code.gs`変更時は既存デプロイIDを維持して新バージョンへ更新。開封Webhookは `setupStepBrevoOpenWebhook` を実行 | 2026-08-13 | 履歴タブを開くと直近10件を新しい順に自動取得。検索／クリア横に更新ボタン。Brevoのopenedイベントを送信要求ID・宛先単位で記録し、履歴に開封確認あり／なし／データなしを表示。Apps Script v51・Webhook登録済み。画像遮断等により未開封を断定しない |
