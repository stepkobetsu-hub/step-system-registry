...
- 本番ブランチ: `main`
- 正本ファイル: `exam_ticket.html`、`exam_ticket.css`、`exam_ticket.js`、`qrcode.js`、`Code.gs`、`ExamTicket.gs`
...
- 現行デプロイ: 既存デプロイIDを維持し、2026-08-19にApps Scriptバージョン57「全県模試受験票・リスニングQR登録」へ更新。
...
- リスニング情報: `全県模試リスニング`を正本とし、年度・回・学年（中1～中3）ごとにURL・ユーザー名・パスワードを保存する。対象の情報が未登録または不完全な場合は登録ポップアップを表示し、選択中の中学生について不足があれば印刷を止める。QRコードは端末内の`qrcode.js`でURLのみを画像化し、認証情報は受験票へ別記する。URLへの認証情報埋め込みや自動ログインは、ブラウザー互換性と認証情報露出を避けるため行わない。
- 受験回切替: 通信中に年度・回を切り替えた場合は古い応答を破棄する。登録済みの回へ移動したときは未登録ポップアップを自動で閉じ、別回の情報を誤表示しない。フロントエンドは `exam_ticket.js?v=20260819-13`。
...
- 本番確認: 2026年度APIで在籍生90名・中学生65名を取得し、登録済み学校コードを既存中学生64名へ反映、未登録校1校を登録画面に表示、API警告0件・受験番号重複0件であることを2026-08-19に確認。フロントエンドは `exam_ticket.js?v=20260819-11`／`exam_ticket.css?v=20260819-8`。塾名・塾コード・生徒コード・中学校コードの4行を同じ高さ・同じ文字サイズに統一し、中学校コード見出しの1行固定、氏名欄の大型化・学年併記を確認。Apps Scriptはv56。
