...
- 一覧には送信日時・送信者・対象・件数・結果を表示する。
- 送信内容列は一覧を縦に長くしないため、通常は **「送信内容を見る」** を表示する。
- 「送信内容を見る」を押すと、実際に送信した本文をその場で展開表示する。
- 画像付き送信の場合は、本文と一緒に送信画像も展開表示する。
- 画像のみ送信の場合は「文章なし（画像のみ送信）」と画像を表示する。
- 新規送信後は履歴を自動再読込し、同じ展開表示を使用する。
...
- 送信履歴から「送信内容を見る」で本文・画像を展開できる仕様を追加。
- 旧送信経路・旧応急ファイルを整理し、現行正本を一本化した。

## 2026-09-14 起動表示・設定画面の改善

- 画面本体と端末キャッシュの講師一覧を先に表示し、Supabaseの一覧・履歴・設定状態は後から取得する構成にした。
- 通常画面にあったLINE送信の初回設定案内を削除した。
- 上部に小さな「設定」ボタンを追加し、「STEP配信システムへ」ボタンの左側へ配置した。
- チャネルアクセストークンの設定画面は「設定」ボタンを押した場合だけ開く。
- 設定画面をHTMLの初期状態から非表示にし、起動時に設定案内が一瞬見える現象を防止した。
- 公開画面で新しい設定ボタン、初期非表示、更新版JavaScriptの配信を確認した。
- 反映PR: https://github.com/stepkobetsu-hub/step-form/pull/9
