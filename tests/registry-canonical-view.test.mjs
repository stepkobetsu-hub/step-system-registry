...
test('添付で指定された32カード表示を正本として固定する',()=>{
  ...
  assert.deepEqual(result.slice(0,2).map(item=>item['システム名']),['請求システム','仕訳・経理：URL管理']);
  ...
  assert.deepEqual(sandbox.sortCustomizedCards(result).slice(0,2).map(item=>item['システム名']),['請求システム','仕訳・経理：URL管理']);
});
