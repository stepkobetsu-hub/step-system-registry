...
test('添付で指定された32カード表示を正本として固定する',()=>{
  ...
  const sandbox={organizeEntryImport:()=>names.map((name,index)=>({'ID':name==='STEP請求書PDF作成・配信システム'?'step-invoice-pdf':name==='時間割マスタ入力'?'teacher-timetable-master-input':`id-${index}`,'システム名':name}))};
