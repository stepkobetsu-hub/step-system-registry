...
- カード順の並べ替え（検索できる1行一覧で移動先番号を直接指定。「先頭へ」・↑↓・ドラッグにも対応。順番はGoogle Sheetへ保存）
...
2026-09-24: 並べ替え画面を検索できる1行一覧に改良し、移動先番号を直接指定できるようにしました。利用者が「うまくいきました」と報告。実際のデプロイ版番号は未確認です。既存URLの開けない問題は、既存デプロイを新バージョンへ更新して利用者側で復旧しました（詳細原因は未確定）。


- Google SheetのL列「表示順」を正本とします。
- 初期値は1〜26で登録済みです。
- Webアプリの「↕ 並べ替え」を押すと並べ替えモードになります。
- 並べ替え画面では名前でカードを探し、「移動先」に1〜件数の番号を入力して直接移動できます。「先頭へ」・↑↓やPCでのドラッグも利用できます。
