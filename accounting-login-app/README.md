...
- カード順の並べ替え（PCはドラッグ、各カードの「先頭へ」・↑↓でも移動。順番はGoogle Sheetへ保存）
...
2026-09-24: GitHubの画面コードに、検索できる1行一覧と移動先番号の直接指定を追加しました。Apps Scriptの本番画面には未反映です。`Index.html` をプロジェクトに反映し、既存デプロイを新バージョンに更新する必要があります。


- Google SheetのL列「表示順」を正本とします。
- 初期値は1〜26で登録済みです。
- Webアプリの「↕ 並べ替え」を押すと並べ替えモードになります。
- PCではカードをドラッグできます。どの端末でもカード右上の「先頭へ」で末尾から一度に先頭へ移動でき、↑↓で隣へ移動できます。
- 変更はGoogle SheetのL列へまとめて保存され、別PCでも同じ順番で表示されます。
- 新しいカードは末尾へ追加されます。
